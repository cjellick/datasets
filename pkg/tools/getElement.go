...
func NextElement(w http.ResponseWriter, r *http.Request) {
	...
	if req.DatasetID == "" {
		http.Error(w, "datasetID is required", http.StatusBadRequest)
		return
	} else if req.PreviousName == "" {
		http.Error(w, "previousName is required", http.StatusBadRequest)
		return
	}
	...
	element, err := d.NextElement(req.PreviousName)
	if err != nil {
		http.Error(w, "element not found", http.StatusNotFound)
		return
	}
